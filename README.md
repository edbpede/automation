...
`edbpede/automation/actions/gate@v1.0.0` with the complete `needs` JSON and the
...
```json
{
  "extends": ["github>edbpede/automation//default.json#v1.0.0"]
}
```

After the checked merge workflow and its policy are validated, append
`github>edbpede/automation//automerge.json#v1.0.0`. Mixed projects can append
`github>edbpede/automation//mixed.json#v1.0.0`. Preserve repository-specific
...
Call `edbpede/automation/actions/merge@v1.0.0` from trusted default-branch
